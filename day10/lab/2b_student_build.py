...
def sql_checker_node(state: CheckerState) -> dict:
    ...
    ~4 lines of code.
    """
    has_where = "WHERE" in state["sql"].upper()
    return {
        "is_safe":      has_where,
        "check_reason": "Has WHERE clause — safe to execute." if has_where
                        else "No WHERE clause — full table scan blocked.",
    }
...
def safe_executor_node(state: CheckerState) -> dict:
    ...
    try:
        ...
        conn.close()
        return {"result": df.to_string(index=False, max_rows=20) if not df.empty else "Query returned 0 rows."}
