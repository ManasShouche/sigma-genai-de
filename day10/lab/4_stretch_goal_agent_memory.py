...
REGION     = "us-east-1"
DB_PATH    = os.path.join(os.path.dirname(__file__), "..", "..", "day9", "shared", "sigma_platform.duckdb")
...
""")
    ...
    # ── STEP 1: Broken pipeline ───────────────────────────────────────────────
    # Bug 1 (runtime KeyError):  df["transaction_ids"] — wrong column name
    # Bug 2 (runtime KeyError):  sort_values("total_amounts") — column renamed to "total_amount"
    # Bug 3 (silent logic bug):  WHERE status = 'COMPLETED' — actual values are lowercase
    #                             → query runs, returns 0 rows, wrong totals, no exception raised
    BROKEN_PIPELINE_V2 = '''\
import duckdb, os
...
def run_daily_summary():
    ...
    # Bug 3 (silent logic): wrong case for status filter
    # Actual values are lowercase — this silently returns 0 rows
    df = conn.execute(
        "SELECT transaction_id, merchant_id, amount, status "
        "FROM silver_transactions WHERE status = \\'COMPLETED\\'"
    ).fetchdf()
    conn.close()

    # Bug 1 (runtime KeyError): "transaction_ids" should be "transaction_id"
    total_txns = df["transaction_ids"].nunique()

    df2 = df.groupby("merchant_id").agg({"amount": "sum"}).reset_index()
    df2.columns = ["merchant_id", "total_amount"]

    # Bug 2 (runtime KeyError): "total_amounts" should be "total_amount"
    top = df2.sort_values("total_amounts", ascending=False).iloc[0]

    print(f"Unique transactions: {total_txns}")
    print(f"Merchants found: {len(df2)}")
    print(f"Top merchant: {top[\\'merchant_id\\']} — ${top[\\'total_amount\\']: .2f}")
...
'''
    ...
    result2["student_reflection"] = {"q1": q1 or "NOT ANSWERED", "q2": q2 or "NOT ANSWERED"}
    with open(log2_path, "w", encoding="utf-8") as f:
        json.dump(result2, f, indent=2, ensure_ascii=False)
    ...
    print(f"\nLogged to {log2_path}. Show healing_log_v2.json + your answers to the trainer.")
