...
""")
    ...
    incident_reporter = Agent(
        ...
        goal="Distil any data quality report into a 6-line Slack message that a VP can read and act on in 20 seconds.",
        backstory="""You were once a senior analyst who wrote 15-page incident reports.
The CTO read your last one, returned it with a single comment: 'Write this in 6 lines
or don't send it.' That moment changed you. Now you are physically incapable of writing
more than 6 lines — every extra word feels like a personal failure.""",
        llm=llm_lite,
        verbose=True,
        allow_delegation=False,
    )
...
*Next review:* {datetime.now().strftime('%Y-%m-') + str(datetime.now().day + 1).zfill(2)}
